...
function PaginaLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const usuariosDemo = {
        "funcionario@waynetech.com": {
          nome: "João Silva",
          nivel: "funcionario",
          email: "funcionario@waynetech.com",
        },
        "gerente@waynetech.com": {
          nome: "Maria Santos",
          nivel: "gerente",
          email: "gerente@waynetech.com",
        },
        "bruce@waynetech.com": {
          nome: "Bruce Wayne",
          nivel: "admin",
        ...
        "alfred@waynetech.com": {
          nome: "Alfred Pennyworth",
          nivel: "admin",
          email: "alfred@waynetech.com",
        },
      }

      const usuario = usuariosDemo[email]

      if (usuario && senha === "wayne123") {
        const token = "token_demo_" + Date.now()
        login(usuario, token)
        navigate("/dashboard")
      } else {
        setMensagem({ tipo: "erro", texto: "Credenciais inválidas" })
      }
    } catch (erro) {
      setMensagem({ tipo: "erro", texto: "Erro ao fazer login" })
    }
  }
    ...
    <Container>
      <FormularioCard initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
        ...
        <Formulario onSubmit={handleSubmit}>
          ...
          <Botao type="submit" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
            ACESSAR SISTEMA
          </Botao>
        </Formulario>
        ...
        <InfoDemo>
          ...
          Funcionário: funcionario@waynetech.com
          <br />
          Gerente: gerente@waynetech.com
          <br />
